...
function ApprovedCollege() {
  const [cookies] = useCookies();
  const location = useLocation();
  const isUsersPage =
    location.pathname === "/college-approval/approved-college-list";
  ...
  const filteredRows = rows.filter((row) => {
    const { collegeName, university, createdBy, status } = row;
    const lowerCaseQuery = searchQuery.toLowerCase();
    return (
      (collegeName && collegeName.toLowerCase().includes(lowerCaseQuery)) ||
      (university && university.toLowerCase().includes(lowerCaseQuery)) ||
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        setRows(response.data.colleges);
        setLoading(false);
      } else {
  ...
  const handleView = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        navigate(`/college-approval/view-approve-list/${id}`, {
          state: { userData },
        });
      } else {
  ...
  const columns = [
    ...
    { id: "item4", label: "University" },
    { id: "item5", label: "Created By" },
...
export default ApprovedCollege;
