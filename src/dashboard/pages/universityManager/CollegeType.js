...
function CollegeType() {
  ...
  const isUsersPage = location.pathname === "/university-manager/college-type";
  ...
  const filteredRows = rows.filter((row) => {
    const { collegeType } = row;
    const lowerCaseQuery = searchQuery.toLowerCase();
    return collegeType && collegeType.toLowerCase().includes(lowerCaseQuery);
  });
  ...
  const handleAddCollegeType = async () => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.addcollegetype,
        params,
        cookies
      );
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        setRows(response.data.collegeTypes);
        setLoading(false);
  ...
  const handleViewEdit = async (id) => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getcollegetype,
        params,
        cookies
      );

      if (response?.status === 200) {
        const userData = response.data.collegeTypes;
        setSelectedRow({
          item1: userData.id,
          item2: userData.collegeType,
          status: "Active",
        });
  ...
  const handleSaveChanges = async () => {
    const params = {
      ...
      collegeType: selectedRow.item2,
      status: "Active",
    };
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.updatecollegetype,
        params,
        cookies
      );
      if (response?.status === 200) {
  ...
  const handleCollegeDelete = async () => {
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.deletecollegetype,
        { id: deleteItemId },
        cookies
      );
      if (response?.status >= 200 && response?.status < 300) {
        if (response.data.responseCode === "200") {
          showSnackbar("College type deleted successfully");
      ...
      <Grid sx={{ display: "flex" }}>
        ...
        <Grid component="main" sx={{ width: "100%", px: 1 }}>
          ...
          <Grid sx={{ py: 1 }}>
            ...
            <Grid sx={{ width: "100%",     px: isMobileScreen ? 1 : isSmallScreen ? 2 : 5,}}>
              <Grid
                container
                md={12}
                xs={12}
