...
function CollegeList() {
  ...
  const isUsersPage = location.pathname === "/university-manager/college-list";
  ...
  const handleView = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        navigate(`/university-manager/view-college/${id}`, {
          state: { userData },
        });
      } else {
  ...
  const handleEdit = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        navigate(`/university-manager/edit-college/${id}`, {
          state: { userData },
        });
      } else {
        showSnackbar("Something went wrong. Please try again later!!");
