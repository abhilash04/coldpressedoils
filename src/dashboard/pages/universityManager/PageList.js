...
function PageList() {
  ...
  const isUsersPage = location.pathname === "/web-settings/page-list";
  ...
  const filteredRows = rows.filter((row) => {
    const { pageTitle, location, ogUrl, status } = row;
    const lowerCaseQuery = searchQuery.toLowerCase();
    return (
      (pageTitle && pageTitle.toLowerCase().includes(lowerCaseQuery)) ||
      (location && location.toLowerCase().includes(lowerCaseQuery)) ||
      (ogUrl && ogUrl.toLowerCase().includes(lowerCaseQuery)) ||
  ...
  const fetchTableData = async () => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getPages,
        params,
        cookies
      );
  ...
  const handleView = async (id) => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getPageByID,
        params,
        cookies
      );

      if (response?.status === 200) {
        const userData = response.data.page;

        navigate(`/settings-manager/view-page/${id}`, {
          state: { userData },
        });
      } else {
  ...
  const handleEdit = async (id) => {
    ...
    try {
      ...
      if (response?.status === 200) {
        ...
        navigate(`/settings-manager/edit-page/${id}`, {
          state: { userData },
        });
      } else {
        showSnackbar("Something went wrong. Please try again later!!");
  ...
  const columns = [
    ...
    { id: "item2", label: "pageTitle" },
    { id: "item3", label: "Location" },
    { id: "item4", label: "ogUrl" },
