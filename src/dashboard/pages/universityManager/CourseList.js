...
function CourseList() {
  ...
  const isUsersPage = location.pathname === "/university-manager/course-list";
  ...
  const filteredRows = rows.filter((row) => {
    ...
    return course && course.toLowerCase().includes(lowerCaseQuery);
  });
  ...
  const handleAddCourse = async () => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.addcourse,
        params,
        cookies
      );
  ...
  const handleViewEdit = async (id) => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getcourse,
        params,
        cookies
      );

      if (response?.status === 200) {
        const userData = response.data.course;
        setSelectedRow({
          item1: userData.id,
          item2: userData.course,
          status: "Active",
        });
  ...
  const handleCourseDelete = async () => {
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.deletecourse,
        { id: deleteItemId },
        cookies
      );
      if (response?.status >= 200 && response?.status < 300) {
