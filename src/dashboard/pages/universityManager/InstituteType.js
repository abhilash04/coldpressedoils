...
function InstituteType() {
  ...
  const filteredRows = rows.filter((row) => {
    const { instituteType } = row;
    const lowerCaseQuery = searchQuery.toLowerCase();
    return (
      instituteType && instituteType.toLowerCase().includes(lowerCaseQuery)
    );
  });
  ...
  const handleAddInstituteType = async () => {
    const params = {
      instituteType: instituteType,
    };
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.addinstitutetype,
        params,
        cookies
      );
  ...
  const fetchTableData = async () => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getinstitutetypes,
        params,
        cookies
      );
      console.log(params);
      if (response?.status === 200) {
        setRows(response.data.instituteTypes);
        setLoading(false);
        console.log(response.data.instituteTypes);
      } else {
  ...
  const handleViewEdit = async (id) => {
    ...
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.getinstitutetype,
        params,
        cookies
      );

      if (response?.status === 200) {
        const userData = response.data.instituteTypes;
        setSelectedRow({
          item1: userData.id,
          item2: userData.instituteType,
          status: "Active",
        });
  ...
  const handleSaveChanges = async () => {
    const params = {
      ...
      instituteType: selectedRow.item2,
      status: "Active",
    };
    try {
      const response = await invokeApi(
  ...
  const handleInstituteDelete = async () => {
    try {
      const response = await invokeApi(
        config.getMyCollege + apiList.deleteinstitutetype,
        { id: deleteItemId },
        cookies
      );
      if (response?.status >= 200 && response?.status < 300) {
        if (response.data.responseCode === "200") {
          showSnackbar("Institute type deleted successfully");
                    ...
                    <DialogContent>
                      <Grid container md={12} xs={12}>
                        <Grid item md={12} xs={12} sx={{ my: 1 }}>
                          ...
                          <TextField
                            placeholder="Enter the institute type"
                            autoFocus
                            fullWidth
                            value={instituteType}
                            onChange={(e) => {
